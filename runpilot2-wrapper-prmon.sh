...
function pilot_cmd() {
  ...
  # fi
  # cmd="${pybin} pilot2/pilot.py -q ${qarg} -i ${iarg} -j ${jarg} ${opt_harvester_datadir} --pilot-user=rubin ${pilotargs}"

  # The option x was added in pilot2-2.12.4.7
  # set the maximum failures to 3 on getjob request
  # set the maximum of getjob requests to 5
  #
  cmd="${pybin} pilot2/pilot.py -q ${qarg} -i ${iarg} -j ${jarg} --pilot-user=rubin -x 3 -v 5 ${pilotargs}"
